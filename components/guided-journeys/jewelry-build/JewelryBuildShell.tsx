...
// ── Jewelry Build journey shell ───────────────────────────────────────────
// Renders the four-session journey overlay. Mirrors the editorial layout of
// SweetsSelectionShell / LogisticsJourneyShell — intro, progress bar,
// expandable session cards, completion banner with three action CTAs.
...
import { BridalInventorySession } from "./BridalInventorySession";
import { GroomInventorySession } from "./GroomInventorySession";
import { FamilyHeirloomsSession } from "./FamilyHeirloomsSession";
import { FittingsCustodySession } from "./FittingsCustodySession";

const INTRO = EXTRA_JOURNEY_INTROS["jewelry:build"];

const SESSION_BODIES: Record<
  JewelryBuildSessionKey,
  () => React.ReactElement
> = {
  bridal_inventory: BridalInventorySession,
  groom_inventory: GroomInventorySession,
  family_heirlooms: FamilyHeirloomsSession,
  fittings_custody: FittingsCustodySession,
};

export function JewelryBuildShell({
  category: _category,
  initialSessionKey,
}: {
  category: WorkspaceCategory;
  initialSessionKey?: JewelryBuildSessionKey;
}) {
  const [state, update] = useCategoryJourneyState(
    JEWELRY_BUILD_CATEGORY,
    JEWELRY_BUILD_JOURNEY_ID,
  );
  const setSession = useJewelryBuildLauncher((s) => s.setSession);
  const close = useJewelryBuildLauncher((s) => s.close);

  const [openKey, setOpenKey] = useState<JewelryBuildSessionKey | null>(
    initialSessionKey ?? firstIncomplete(state.sessionStatus),
  );

  const sessionStatuses = useMemo(
    () =>
      JEWELRY_BUILD_SESSIONS.map((s) => ({
        ...s,
  ...
  const markComplete = useCallback(
    (key: JewelryBuildSessionKey) => {
      ...
      const idx = JEWELRY_BUILD_SESSIONS.findIndex((s) => s.key === key);
      const next = JEWELRY_BUILD_SESSIONS.slice(idx + 1).find(
        (s) =>
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative" }}>
        ...
        >
          {allDone
            ? "Every piece tracked, every handoff planned, insurance locked."
...
function CompletionBanner({ onClose }: { onClose: () => void }) {
  ...
  const bridalCount = countPieces(state.formData["bridal_inventory"]);
  const groomCount = countPieces(state.formData["groom_inventory"]);
  const heirloomCount = countHeirlooms(state.formData["family_heirlooms"]);
  ...
  return (
    ...
    >
      ...
      >
        Your jewelry is tracked, paired, and protected.
      </h3>
      <p
        style={{
          fontFamily: FONT_SERIF,
      ...
      >
        {bridalCount} bridal piece{bridalCount === 1 ? "" : "s"} · {groomCount}{" "}
        groom piece{groomCount === 1 ? "" : "s"} · {heirloomCount} heirloom
        {heirloomCount === 1 ? "" : "s"} · custody chain locked.
      </p>
      <div
        style={{
          display: "inline-flex",
      ...
      >
        <CompletionButton primary>
          <Share2 size={12} strokeWidth={2} /> Share custody plan with planner
        </CompletionButton>
        <CompletionButton>
          <Send size={12} strokeWidth={2} /> Send pairing guide to stylist
        </CompletionButton>
        <CompletionButton>
          <FileDown size={12} strokeWidth={2} /> Export insurance manifest
        </CompletionButton>
        <button
          type="button"
          onClick={onClose}
...
function countPieces(bag: unknown): number {
  if (!bag || typeof bag !== "object") return 0;
  const pieces = (bag as { pieces?: unknown[] }).pieces;
  return Array.isArray(pieces) ? pieces.length : 0;
}

function countHeirlooms(bag: unknown): number {
  if (!bag || typeof bag !== "object") return 0;
  const heirlooms = (bag as { heirlooms?: unknown[] }).heirlooms;
  return Array.isArray(heirlooms) ? heirlooms.length : 0;
}

// ─── Session card (mirrors SweetsSelectionShell) ─────────────────────────
...
function SessionCard({
  ...
  session: JewelryBuildSessionDef & { status: GuidedSessionStatus };
  expanded: boolean;
  isFinal: boolean;
  onToggle: () => void;
  onMarkComplete: () => void;
          ...
          >
            ...
            >
              {session.status === "completed"
                ...
                : session.status === "in_progress"
                  ? isFinal
                    ? "When the custody plan looks right, lock it in."
            ...
            {session.status === "in_progress" && (
              ...
              >
                {isFinal ? "Lock in custody plan →" : "I'm happy with this →"}
              </button>
            )}
            {session.status === "completed" && (
              <button
