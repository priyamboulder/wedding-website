...
// Hosts two tabs — "Tables" (existing table management UI, slimmed) and
// "Guests" (new — drag/drop assignment). Shares the same collapse/expand
// affordance as the previous TableSidebar.
...
import { GuestsSidebar } from "./GuestsSidebar";
...
type Tab = "elements" | "tables" | "guests";
...
export function CombinedSidebar({
  ...
  const assignedCount = assignmentForEvent?.length ?? 0;
  ...
  return (
    <aside className="flex h-full w-[320px] flex-shrink-0 flex-col overflow-hidden rounded-lg border border-border bg-white">
      ...
      {tab === "elements" ? (
        <ElementsSidebar onAddElement={onAddElement ?? (() => undefined)} />
      ) : tab === "tables" ? (
        <TablesTab capacity={capacity} />
      ) : (
        <GuestsSidebar guests={guests} eventId={eventId} />
      )}
    </aside>
  );
}
...
// ── Tables tab (extracted from the old TableSidebar) ────────────────
function TablesTab({ capacity }: { capacity: number }) {
  const tables = useSeatingStore((s) => s.tables);
...
function TableRow({
  ...
  const eventId = useSeatingAssignmentsStore((s) => s.activeEventId);
  const assignmentForEvent = useSeatingAssignmentsStore(
    (s) => s.assignments[eventId],
  );
  const occupied = (assignmentForEvent ?? []).filter(
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-1 font-mono text-[9.5px] text-ink-faint">
          {occupied} / {table.seats} assigned
          {occupied === table.seats && occupied > 0 && (
            <span className="ml-1 text-sage">· full</span>
          )}
        </div>
      </div>
