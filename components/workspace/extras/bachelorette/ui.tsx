...
// ── Shared UI atoms for Bachelorette tabs ──────────────────────────────────
// Small building blocks the five tabs share — eyebrowed section frames,
// small-caps labels, inline add-item inputs. Kept co-located with the tabs
// rather than promoted to the global primitives because they encode the
// Memories & Keepsakes section styling specifically.
...
export function Section({
  ...
  return (
    <section
      className={cn("rounded-lg border p-5", border, bg)}
    >
...
export function ChipList({
  ...
  if (items.length === 0 && emptyLabel) {
    return (
      <p className="text-[12.5px] italic text-ink-faint">{emptyLabel}</p>
    );
  }
  return (
