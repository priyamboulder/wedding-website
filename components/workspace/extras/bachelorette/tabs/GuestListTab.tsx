...
// ── Guest List & RSVP tab ──────────────────────────────────────────────────
// Table of guests with inline role / RSVP / room edits. Roster strip above
...
import { RSVP_OPTIONS } from "@/lib/bachelorette-seed";
import { useBacheloretteStore } from "@/stores/bachelorette-store";
import type { Guest, RsvpStatus } from "@/types/bachelorette";
...
function RosterStrip() {
  ...
  return (
    <Section
      ...
      description="Track RSVPs, roles, and room share. Send the RSVP link to update this table automatically."
      right={
...
function CountCard({
  ...
  const color =
    tone === "sage" ? "text-sage" : tone === "gold" ? "text-gold" : "text-rose";
  return (
...
function GuestTable() {
  const guests = useBacheloretteStore((s) => s.guests);
  const rooms = useBacheloretteStore((s) => s.rooms);
  const addGuest = useBacheloretteStore((s) => s.addGuest);
  const updateGuest = useBacheloretteStore((s) => s.updateGuest);
  const removeGuest = useBacheloretteStore((s) => s.removeGuest);
  const setGuestRsvp = useBacheloretteStore((s) => s.setGuestRsvp);
  const assignGuestToRoom = useBacheloretteStore((s) => s.assignGuestToRoom);

  return (
    <Section title="Who's invited">
...
function Th({
  ...
  return (
    <th
      className={cn(
        "px-4 py-2 font-mono text-[10px] font-medium",
        className,
      )}
...
function AddGuestRow({
  ...
  function commit() {
    if (!name.trim()) return;
    onAdd(name.trim(), role.trim() || "Guest");
    setName("");
    setRole("");
  }
  return (
...
function RoomAssignments() {
  const rooms = useBacheloretteStore((s) => s.rooms);
  const guests = useBacheloretteStore((s) => s.guests);
  const addRoom = useBacheloretteStore((s) => s.addRoom);
  const updateRoom = useBacheloretteStore((s) => s.updateRoom);
  const removeRoom = useBacheloretteStore((s) => s.removeRoom);
