...
// ── Budget & Splits tab ────────────────────────────────────────────────────
// The best man's most common pain point — handled transparently. Expenses
// with per-line split methods, a splitting rule for the groom, per-guest
// payment tracker, and a private organizer-only note lane.
...
export function BudgetTab() {
  ...
  const expenses = useBachelorStore((s) => s.expenses);
  const addExpense = useBachelorStore((s) => s.addExpense);
  return (
...
function BudgetSummary() {
  const expenses = useBachelorStore((s) => s.expenses);
  const guests = useBachelorStore((s) => s.guests);
  const payments = useBachelorStore((s) => s.payments);
  const budget = useBachelorStore((s) => s.budget);
    ...
    >
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <SummaryCard
          label="Groom pays"
          value={
            budget.splittingRule === "groom_pays_own"
              ? formatMoney(perPersonCents)
              : "$0"
          }
          hint={
            budget.splittingRule === "groom_pays_own"
              ? "Groom splits equally"
              : "Covered by crew"
          }
          tone="rose"
        />
...
function ExpenseTable() {
  const expenses = useBachelorStore((s) => s.expenses);
  const addExpense = useBachelorStore((s) => s.addExpense);
  const updateExpense = useBachelorStore((s) => s.updateExpense);
  const removeExpense = useBachelorStore((s) => s.removeExpense);
  const guests = useBachelorStore((s) => s.guests);
    ...
    <tr>
      ...
      <td className="px-4 py-2">
        ...
        >
          ...
          <option value="split_among_guests">
            Split among crew (groom covered)
          </option>
...
function SplittingRuleControl() {
  const rule = useBachelorStore((s) => s.budget.splittingRule);
  const setSplittingRule = useBachelorStore((s) => s.setSplittingRule);

  const options: { value: SplittingRule; label: string; hint: string }[] = [
    {
      value: "equal",
      label: "Everyone splits equally",
      hint: "Groom's share divided among the crew",
    },
    {
      value: "groom_pays_own",
      label: "Groom pays his own way",
      hint: "Each guy — groom included — pays his share",
    },
    {
      value: "custom",
      label: "Custom split",
...
function PaymentTracker() {
  ...
  const payments = useBachelorStore((s) => s.payments);
  const budget = useBachelorStore((s) => s.budget);
  const recordPayment = useBachelorStore((s) => s.recordPayment);
  const setPaymentStatus = useBachelorStore((s) => s.setPaymentStatus);
  ...
  return (
    <Section
      ...
      right={
        ...
        >
          <Bell size={12} strokeWidth={1.8} /> Nudge the stragglers
        </button>
      }
    >
            ...
            {going.length === 0 && (
              <tr>
                <td
                  ...
                  className="px-4 py-6 text-center text-[12.5px] italic text-ink-faint"
                >
                  Add crew to the Guest List first.
                </td>
              </tr>
            )}
          </tbody>
...
function OrganizerNotes() {
  const notes = useBachelorStore((s) => s.organizerNotes);
  const addNote = useBachelorStore((s) => s.addOrganizerNote);
  const updateNote = useBachelorStore((s) => s.updateOrganizerNote);
  const removeNote = useBachelorStore((s) => s.removeOrganizerNote);
  ...
  return (
    <Section
      ...
      title="Best-man only — the groom never sees these"
    ...
    >
      ...
      <div className="mt-3 flex items-start gap-2">
        <TextArea
          value={draft}
          onChange={setDraft}
          placeholder="Add a best-man-only note…"
          rows={2}
        />
        <button
          type="button"
...
function splitPerPerson(expense: Expense, going: number): number | null {
  ...
  switch (split.kind) {
    ...
    case "split_among_guests": {
      // Groom's expense — divided among the rest of the crew
...
function computePerPersonCents(
  ...
  let perPerson = 0;
  const groomPaysShare = rule !== "equal";
  for (const e of expenses) {
    const pp = splitPerPerson(e, going.length);
    if (pp === null) continue;
    if (e.split.kind === "split_among_guests" && groomPaysShare) {
      continue;
    }
    perPerson += pp;
  }
