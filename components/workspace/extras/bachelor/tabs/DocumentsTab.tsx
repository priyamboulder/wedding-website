...
// ── Documents tab ──────────────────────────────────────────────────────────
// Lightweight document vault for the bachelor — booking confirmations,
// reservations, flight itineraries, receipts, and group photos. Each row
// stores a label, category, and optional URL. No backend — users paste links.
...
export function DocumentsTab() {
  const documents = useBachelorStore((s) => s.documents);
  const addDocument = useBachelorStore((s) => s.addDocument);
  const updateDocument = useBachelorStore((s) => s.updateDocument);
  const removeDocument = useBachelorStore((s) => s.removeDocument);
    ...
    <Section>
      ...
      <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-[1fr_1fr_180px_auto]">
        <TextInput
          value={label}
          onChange={setLabel}
          placeholder="What is it? (e.g. Stadium Course tee time)"
        />
...
function DocumentRow({
  ...
  doc: BachelorDocument;
  onUpdate: (patch: Partial<BachelorDocument>) => void;
  onRemove: () => void;
}) {
