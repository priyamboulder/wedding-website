...
export type MonogramExportFormat = "svg" | "png-transparent" | "png-ivory" | "pdf";
...
export function buildFilename(opts: {
  ...
  return `${opts.coupleSlug}-${opts.templateSlug}-monogram.${opts.ext}`;
}
...
async function svgToPdf(svg: SVGElement, filename: string): Promise<Blob> {
...
export async function exportMonogram(opts: {
  svgNode: SVGElement | null;
  format: MonogramExportFormat;
  coupleSlug: string;
  templateSlug: string;
}): Promise<void> {
  const { svgNode, format, coupleSlug, templateSlug } = opts;
  if (!svgNode) throw new Error("Monogram SVG node not available");
  ...
  if (format === "svg") {
    ...
    triggerDownload(blob, buildFilename({ coupleSlug, templateSlug, ext: "svg" }));
    return;
  }

  if (format === "png-transparent") {
    const blob = await svgToPng(svgNode, 2048, null);
    triggerDownload(blob, buildFilename({ coupleSlug, templateSlug, ext: "png" }));
    return;
  }
  ...
  if (format === "png-ivory") {
    ...
    triggerDownload(
      blob,
      buildFilename({ coupleSlug, templateSlug, ext: "png" }).replace(
        /-monogram\.png$/,
        "-monogram-ivory.png",
      ),
    );
  ...
  if (format === "pdf") {
    const filename = buildFilename({ coupleSlug, templateSlug, ext: "pdf" });
    const blob = await svgToPdf(svgNode, filename);
